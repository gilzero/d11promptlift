...
radix_promptlift theme uses [Webpack](https://webpack.js.org) to compile and
bundle SASS and JS.
...
Go to the root of radix_promptlift theme and run the following commands:
`nvm use`
...
`drush en radix_promptlift -y`
`drush config-set system.theme default radix_promptlift -y`
